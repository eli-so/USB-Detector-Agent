...
class data_handler:
    ...
    def __init__(self,WMIinternalPNPObjectProperties = None ,action = None ,offline_data = None ,report_pipeline = ["elasticsearch"]):
        """
        Create Data Handler object
        This Object supporting 2 push method
        1. push data to ELK via Elasticsearch package
        *  while usuing this method , all data saved first in offline_data object and send if the ELK host is reachable
        in case of connectivity issue , the data saved in offline_data object and will push once the Host will be reachable
    ...
    def report_data(self):
        ...
        if self.WMIinternalPNPObjectProperties.Service[1] == "USBSTOR" and self.action == "Device_Added" :
            friendlyname = get_disk_drive_name(deviceid=self.WMIinternalPNPObjectProperties.PNPDeviceID[1].split("\\")[-1])
        self.data = {
            ...
            "FriendlyName": self.WMIinternalPNPObjectProperties.Name[1],
    ...
    def _report_to_elastic_search(self):
        try:
            es = Elasticsearch([{'host': CONFIG.ESHOST, 'port': CONFIG.ESPORT}])
            if es.ping():
